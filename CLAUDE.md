...
- **Cloudflare R2**: the account holds three buckets — `kandaka-library`, `nilebookstore-books`, and `nile-comics`. **`kandaka-library` is Kandaka's own bucket and the correct target for new library uploads** (public base URL `https://pub-0c3bb637d5f54b239fe4a4ef9b08bfe3.r2.dev` — confirmed by direct fetch, and matches the "switched to kandaka-library as of Batch 5, July 2026" note in `upload_new_batch.py`). `nilebookstore-books` (public base URL `https://pub-a5e3b47fe87749f491660d68e2029284.r2.dev`) belongs to a **separate, independent site/project** (Nile Book Store) — most of `data/library.json`'s 500+ legacy entries happen to point at it from earlier batches, but it is not Kandaka's bucket and new uploads should not go there. Always check `data/library.json` per-entry rather than assuming one bucket URL — older entries use the `nilebookstore-books` (`pub-a5e3...`) URL, newer ones use `kandaka-library` (`pub-0c3bb637...`). (Corrected August 2026 — a prior version of this note had the two backwards/conflated; verified by fetching known files directly from each URL.)
