# Kandaka Workplan

> Persistent editorial and research tracker. This file records decisions, drafts, queues, source-acquisition work, website changes, and publication readiness. It does **not** authorize publication.

**Last updated:** 2026-09-19  
**Publication rule:** Nothing may be translated, scheduled, committed as publishable content, or published without explicit approval of that specific article.  
**Current overall state:** Planning and review only; no item in this file is approved for publication unless its `Approved` field is explicitly marked `Yes`.

## How to maintain this file

Use these standard fields for every article, Library summary, acquisition, and site change:

- **Research:** Not started / In progress / Complete / Refresh required
- **Draft:** Not started / Outline / Partial / Complete
- **Review:** Not ready / Awaiting review / Changes requested / Complete
- **Approved:** No / Yes
- **Published:** No / Scheduled YYYY-MM-DD / Yes YYYY-MM-DD
- **Next action:** One concrete next step

When adding an item, preserve the approved thesis and requirements; append new decisions under `Notes/requirements` rather than silently replacing earlier ones. Dates are proposals until the article is individually approved.

## Editorial rules already established

1. Kandaka should be empirically grounded, historically literate, readable by an intelligent general audience, and willing to challenge fashionable orthodoxy.
2. Do not automatically agree with an article idea. Test its priority, feasibility, cost, opportunity cost, institutional demands, and alternatives. Reject or lower the priority of weak proposals.
3. Favor value addition over raw commodity exports; local ownership and capability-building; strategic but accountable state action; deliberate sequencing; decentralization; maintenance of existing assets; and measurable performance.
4. International contractors and imports should transfer skills, documentation, maintenance capacity, and a pathway toward local production.
5. Do not romanticize either the state or markets. Assess what worked, what failed, why it failed, and who benefited.
6. Treat peace, inclusion, regional equality, women and youth employment, accessibility for elderly and disabled people, and environmental/community safeguards as development requirements rather than footnotes.
7. Distinguish claims from evidence. In particular, distinguish mineral occurrences, resources, reserves, recoverability, and commercial feasibility.
8. Prefer original data, plans, surveys, evaluations, feasibility studies, and primary documents. Press releases may inform research but ordinarily do not belong in the Library.
9. Before selecting a Library work for a full summary, inventory existing Kandaka summaries and verify that the work has not already received the full analytical treatment.
10. Every substantive factual claim, number, comparison, and current organization recommendation must be checked against the underlying source and refreshed close to publication.

## Approved editorial structure

The intended top-level structure is:

| Category | Scope | Status |
|---|---|---|
| Ideas | Political/social thought, religion, identity, and conceptual essays | Existing |
| Development | Economics, infrastructure, industry, agriculture, education, monetary policy, and planning | Existing |
| History | Funj, Kush, Nubia, colonial Sudan, historical people, archaeology, and historical images | **Approved; not yet implemented** |
| Culture | Wrestling, sport, literature, film, traditions, arts, and culture-led tourism | **Approved; not yet implemented** |
| News | Current reporting | Existing |
| Library | Books, papers, reports, and document summaries | Existing |

Use tags for narrower subjects rather than creating many new top-level categories. The desired navigation order is **Ideas · Development · History · Culture · News · Library**. The homepage feature should rotate across Ideas, Development, History, Culture, and selected archival/images material.

## Proposed publication cadence and workflow

**Proposed cadence:** One substantive article every three days, alternating subjects so Kandaka does not become economics-only. The earlier provisional sequence was September 1, 4, 7, and 10, 2026; those dates have passed and must be rescheduled rather than reused automatically.

Required workflow for each article:

1. Confirm the user's thesis and non-negotiable requirements.
2. Research and fact-check the English article.
3. Produce a complete English review draft marked `DRAFT — NOT APPROVED FOR PUBLICATION`.
4. Obtain explicit, article-specific approval.
5. Make final corrections.
6. Produce the Arabic edition.
7. Prepare English and Arabic site files, metadata, licensed images, and social preview.
8. Schedule only after approval; verify the live publication and promotion.

Approval of one article never approves another. If an article is not ready or approved, move its date; do not publish an unfinished piece merely to preserve cadence.

## Active article queue

### A01 — Sudan's Next Five-Year Plan: Development Without Waiting for Rescue

- **Category/tags:** Development; industrial policy, planning, decentralization, employment, trade
- **Priority:** Highest
- **Research:** Complete, but time-sensitive 2026 figures require a final source check before publication
- **Draft:** Complete English draft
- **Review:** Awaiting review
- **Approved:** No
- **Published:** No
- **Core thesis:** Sudan should plan from productive self-reliance—beginning with the imports that cost the most foreign currency and can realistically be replaced at home—without depending on rescue, aid, or external finance that may not arrive.
- **Requirements:** Rank sectors by dollars saved, five-year feasibility, jobs (especially women and youth), and capacity for later exports. Separate replaceable consumption imports, structurally reducible demand, and productive capital imports. Include realistic—not total—wheat substitution; repair existing sugar and industrial capacity; edible oils, textiles, livestock, medicines, solar, and selected manufacturing; State Production Compacts; enforceable support/performance agreements; a national scorecard; gold/mineral accountability; local capability; and lessons from Sudan and comparator countries. Employment is an explicit target, not a hoped-for side effect.
- **Image:** Use an authentic image of productive Sudan—preferably solar irrigation/agriculture, Sudanese manufacturing, or a strong historical-development image. Verify license and attribution. It must work as the article banner, social preview, and Pinterest source image.
- **Next action:** Locate the latest clean draft, update/verify every time-sensitive statistic and link, then deliver a single review copy.

### A02 — Sudan Planned Before: What Went Right, What Went Wrong, and What We Must Learn

- **Category/tags:** Development; History, accountability, institutions, planning
- **Priority:** Highest
- **Research:** Substantially complete; final quantified timeline and fact-check required
- **Draft:** Complete first draft
- **Review:** Awaiting review after final polish
- **Approved:** No
- **Published:** No
- **Core thesis:** Sudan did produce serious plans and sometimes built valuable assets, but no post-independence political order sustained a coherent, inclusive development strategy with the institutional discipline, financing, continuity, peace, and accountability required for transformation.
- **Requirements:** Begin with the mixed colonial baseline without romanticizing it. Assess early independence, Abboud, Nimeiri, democratic periods, Bashir/Turabi, and the post-2019 transition using the same scorecard. Cover war as development policy in reverse; debt; corruption and lack of consequences; coups and failed peaceful transition; over-centralization; implementation capacity; successes and failures; Sudan Railways and Sudan Airways; education, unequal access, and brain drain; failure to build, maintain, and retain; and conclude with rules the next plan must never repeat. Separate evidence from personal judgment and acknowledge genuine achievements by disliked governments and genuine failures by favored ones.
- **Next action:** Complete the quantified timeline, confirm source citations, tighten the final lessons, and deliver the English review draft.

### A03 — Ancient and Modern Wrestling in Sudan

- **Category/tags:** Culture; History, Nuba, sport, national identity
- **Priority:** High
- **Research:** Complete enough for review; sources need final citation check
- **Draft:** Complete English draft reported in the conversation
- **Review:** Awaiting review
- **Approved:** No
- **Published:** No
- **Core thesis:** Sudan should recognize wrestling as a living cultural and sporting institution, carefully examine—but not overclaim—the possible continuity between ancient Nubian athletic evidence and modern Nuba wrestling, and build a national pathway from schools and local traditions to elite competition.
- **Requirements:** Cover ancient depictions and what they do and do not prove; modern Nuba wrestling, ceremony, community, and prestige; its spread into Khartoum and wider Sudan; the wars and marginalization of the Nuba Mountains without reducing the culture to folklore; why existing federation/school efforts never scaled; schools, academies, girls' participation, coaches, referees, nutrition, scholarships, state leagues, national festivals, African competition, and an Olympic pathway. Use Dagestan/Chechnya as comparisons, not templates. Sport should support inclusive national identity.
- **Image:** Pair credible ancient visual evidence with an authentic, properly licensed modern Sudanese/Nuba wrestling image. Avoid implying a proven uninterrupted lineage.
- **Next action:** Locate and consolidate the full draft, verify all historical and Olympic claims, then deliver it for review.

### A04 — From Herds to Industry: How Sudan Could Become a World Livestock Power

- **Category/tags:** Development; livestock, fisheries, industry, cooperatives, regional development
- **Priority:** High
- **Research:** Complete enough for review; current statistics require refresh
- **Draft:** Complete English draft reported in the conversation
- **Review:** Awaiting review
- **Approved:** No
- **Published:** No
- **Core thesis:** Sudan should retain profitable live-animal exports where sensible while capturing much more value domestically through locally owned livestock and fisheries industries.
- **Requirements:** Cover cattle, sheep, goats, camels, meat, dairy, leather and finished goods, wool, feed, veterinary medicine and vaccines, breeding/genetics, slaughterhouses, cold chains, rendering, fertilizer, gelatin/collagen, pet food, logistics, Red Sea fisheries, inland/Nile fisheries, and environmentally/economically appropriate aquaculture. Establish regional hubs in underdeveloped states. Give pastoralists and local producer groups genuine equity or profit-sharing when it increases participation and returns. Explicitly include women, youth, ethnic and religious minorities, and marginalized communities. Address export standards, animal health, sustainability, market risk, and five-year targets.
- **Image:** Authentic Sudanese pastoralists/livestock/fisheries or local processing—not generic global livestock photography; confirm license and attribution.
- **Next action:** Locate and consolidate the full draft, update current herd/export evidence, then deliver it for review.

### A05 — How Should Sudan Rebuild Trust in the Pound?

- **Subtitle:** Gold, Dollarization and the Search for Monetary Stability
- **Category/tags:** Development; monetary policy, gold, inflation, dollarization
- **Priority:** High
- **Research:** Complete enough for consolidation; time-sensitive evidence requires refresh
- **Draft:** Substantial draft plus completed conclusion; needs one clean consolidated draft
- **Review:** Not ready
- **Approved:** No
- **Published:** No
- **Core thesis:** Sudan probably should not adopt a classical gold standard or permanently dollarize unless trust in a national currency proves impossible to rebuild. The strongest option to investigate is a legally constrained Sudanese pound backed during stabilization by independently audited gold and diversified liquid reserves, with a realistic exchange rate, central-bank independence, and strict limits on routine deficit financing.
- **Requirements:** Fairly compare classical gold, diversified reserve backing, a currency board, dollarization/other foreign currencies, parallel currencies, and eventual inflation targeting. Cover dollar/euro/RMB/Gulf/African payment alternatives; geopolitical diversification; gold-sector transparency; fiscal discipline; emergency-financing rules; published reserves and government borrowing; independent audits; redenomination only alongside stabilization; stable money as development infrastructure; and lessons from Brazil, Israel, Bulgaria, Estonia, Peru, Georgia, Ecuador, Zimbabwe, Botswana, and Morocco. The article must test the gold idea, not assume it.
- **Image:** Preferred cover is the first Sudan/gold image previously selected, subject to locating it and verifying source/license. Prefer authentic Sudanese gold weighing/processing and Sudanese pound notes. Avoid generic Western gold imagery. Pinterest concept: `Can Gold Save the Sudanese Pound?`
- **Next action:** Merge all drafted sections into one coherent English version, refresh evidence, attach verified image details, and deliver for review.

### A06 — Beyond the Car: How Sudan Should Rebuild Its Cities Around Public Transport

- **Subtitle:** BRT, Trambuses, Light Rail, Climate-Resilient Streets and a Sudanese Transit Industry
- **Category/tags:** Development; transport, urban planning, accessibility, industry, climate resilience
- **Priority:** High
- **Research:** Thesis and substantial research complete
- **Draft:** Not started
- **Review:** Not ready
- **Approved:** No
- **Published:** No
- **Core thesis:** Sudan should rebuild cities around the locally maintainable, climate-resilient, universally accessible public-transport system that delivers the most mobility per dollar—not around private-car dependence or a predetermined rail technology.
- **Requirements:** Be national, not Khartoum-only; assess Port Sudan, Greater Khartoum, Wad Madani, Kassala, Gedaref, El Obeid, Atbara, Nyala, and other cities by demand. Evaluate electric buses, dedicated BRT, trolleybuses/trambuses, surface light rail, elevated light metro, underground metro only where justified, regional rail, feeders, walking, and limited/contextual cycling. High-capacity transit must have dedicated right-of-way. Pair transport with land use and dense mixed-use neighborhoods. Establish **5–10 minute essential-services neighborhoods**: homes, schools, clinics, groceries, worship, parks, basic government services, and markets locally accessible, with higher-capacity transit linking major jobs, universities, hospitals, and regional transport.
- **Climate/accessibility requirements:** Design for 45°C+ heat, dust/sandstorms, seasonal flooding, unreliable power, continuous shade, drainage, raised electrical systems, resilient HVAC/filtration, backup power, sheltered transfers, seating, water, safe crossings, low floors/level boarding, wheelchair space, tactile guidance, visual and audible information, accessible toilets, feeder/paratransit, elderly people, disabled people, parents, and people with health limitations. Measure climate-adjusted accessibility and the share of residents who can reach frequent transport in a safe, shaded, accessible 5–10 minute journey.
- **Industry requirements:** Phase localization from bodies, shelters, seats, steelwork, cables, ticketing, and maintenance consumables to vehicle assembly, battery packs, chargers, HVAC, electronics, axles/rail components, and later traction/control systems where justified. Require training, documentation, diagnostic access, spares, technology transfer, and local production pathways. Study Egypt, Turkey/Bozankaya, Morocco, Addis Ababa's warnings, and relevant Sudanese urban scholarship.
- **Next action:** Draft the full English article using the established four tests: affordable, climate-resilient, locally maintainable, and universally accessible.

### A07 — Should Sudan Restore Its Independence Flag?

- **Category/tags:** Ideas; History, identity, constitutional settlement
- **Priority:** Medium as identity/reconciliation; low as economic policy
- **Research:** Preliminary thesis complete
- **Draft:** Not started
- **Review:** Not ready
- **Approved:** No
- **Published:** No
- **Core thesis:** The 1956 blue-yellow-green flag is distinctive, historically Sudanese, and a credible candidate for restoration during a broadly legitimate post-war constitutional reset; the strongest option is restoration essentially unchanged, with only technical standardization of colors/proportions if needed.
- **Requirements:** Present the case for and against restoration. Explain the independence history and the current flag's Pan-Arab family without claiming the current flag has no accumulated national meaning. Do not pretend a flag change creates jobs, peace, or institutional reform. Avoid unnecessary new symbols. Frame restoration as part of constitutional reconciliation, not an urgent development intervention.
- **Next action:** Research authoritative flag history and public/constitutional considerations, then outline the article.

### A08 — Should Sudan Ever Change Its Country Name?

- **Category/tags:** Ideas; identity, history, constitutional settlement
- **Priority:** Low-to-medium; do not treat as a current national priority
- **Research:** Preliminary concept only
- **Draft:** Not started
- **Review:** Not ready
- **Approved:** No
- **Published:** No
- **Core thesis:** After South Sudan's independence, Kandaka can investigate whether `Sudan` still adequately distinguishes the republic, but should not presume renaming is desirable and should not propose `Kush` as the default answer.
- **Requirements:** Separate a website search/filtering problem from the national identity question. Cover the history of the name Sudan and international renaming examples. Any candidate must be inclusive across regions, ethnicities, and religions; workable in Arabic and English; historically/geographically defensible; internationally distinctive; and practical for diplomacy, passports, and trade. Treat exclusion risk and transition costs seriously. Changing symbols without changing institutions is cosmetic.
- **Next action:** Research the name's history and develop neutral evaluation criteria before considering any candidates.

### A09 — How Can You Actually Help Sudan? Following Aid From Donation to Delivery

- **Category/tags:** News or Ideas; humanitarian aid, accountability, practical guide
- **Priority:** High practical value; must be current at publication
- **Research:** Not started / refresh required at publication
- **Draft:** Not started
- **Review:** Not ready
- **Approved:** No
- **Published:** No
- **Core thesis:** Readers need a transparent, evidence-based guide to getting money and assistance to Sudan, evaluating both Sudanese grassroots organizations/Emergency Response Rooms and international organizations by delivery rather than brand recognition.
- **Requirements:** Evaluate financial transparency, program share, verified Sudan operations, local partnerships, geographic reach, safe money transfer, sanctions/compliance constraints, independent reporting, and evidence of delivery. Organize by food, medical care, shelter, displaced families, children/orphans and alternative family care, women and survivors of sexual violence, education, water/sanitation, and direct/community aid. Do not label an organization trustworthy without current verification. Clearly date the research and explain uncertainty/access limitations.
- **Next action:** At drafting time, conduct a fresh organization-by-organization audit and build a transparent comparison table.

### A10 — Women, Children and Security: Does Sudan Need a Specialized Protection Police Service?

- **Category/tags:** Ideas; institutions, policing, gender, child protection
- **Priority:** High social/institutional importance
- **Research:** Not started
- **Draft:** Not started
- **Review:** Not ready
- **Approved:** No
- **Published:** No
- **Core thesis:** More policewomen may improve reporting and trust, but an effective response to rape and child abuse requires specialized women-and-child protection units embedded in a complete survivor-protection system.
- **Requirements:** Examine recruitment and authority of policewomen; trained investigators; confidential and accessible reporting; survivor-centered interviewing; forensic capacity; shelters; social workers; child-protection services; prosecutors; medical care; evidence preservation; witness protection; oversight and accountability; rural/mobile access; and safeguards against abuse by security services. Do not present adding women to a broken institution as sufficient reform.
- **Next action:** Research Sudanese law/institutions, conflict-related sexual violence and child-protection needs, and comparable specialized units.

### A11 — What Megaprojects Could Actually Transform Sudan?

- **Category/tags:** Development; infrastructure, energy, minerals, opportunity cost
- **Priority:** High, but only with strict comparative economics
- **Research:** Not started; uranium source reviewed preliminarily
- **Draft:** Not started
- **Review:** Not ready
- **Approved:** No
- **Published:** No
- **Core thesis:** Megaprojects must compete on development return, not spectacle. A technically feasible project can still be the wrong use of scarce capital, skilled labor, institutional capacity, and political attention.
- **Requirements:** Compare dams, nuclear power, uranium development, national rail electrification, ports, irrigation, desalination, grid expansion, solar corridors, water storage/transfer, industrial cities, high-speed transport, and major afforestation where evidence supports inclusion. Score cost, construction time, foreign currency, local jobs/capability, imports reduced/exports created, people benefiting, maintenance, environmental/social harm, security/regulation, expected economic return, and explicit opportunity cost.
- **Uranium/nuclear requirement:** Separate `Can Sudan become a uranium producer?` from `Should Sudan build nuclear power?` The reviewed GRAS presentation documents occurrences and elevated uranium in Uro phosphate but not proven commercial reserves, recoverable tonnes, costs, or mine feasibility. Investigate UNFC classification, exploration needs, byproduct recovery, environmental and community effects—especially in the Nuba Mountains—and local ownership/revenue/decision-making. Having uranium does not itself make nuclear electricity economical. Compare nuclear with solar, transmission, rail, irrigation, livestock, gold formalization, and other opportunities.
- **Next action:** Build the common scorecard, shortlist only plausible projects, and commission/update cost and capacity evidence before drafting.

## Additional article backlog from this conversation and uploaded idea bank

These items were mentioned or queued but do not yet have fully established theses. They must still use the standard status fields when promoted to the active queue.

| ID | Working idea | Likely category | Research | Draft | Review | Approved | Published | Next action |
|---|---|---|---|---|---|---|---|---|
| B01 | Ethiopian Airlines: Lessons for Rebuilding Sudan Airways | Development | Not started | Not started | Not ready | No | No | Recover the source note and define the Sudan Airways comparison |
| B02 | Kush in the Bible: Sudan in the Biblical World | History | Not started | Not started | Not ready | No | No | Recover source note; separate textual evidence, interpretation, and national mythology |
| B03 | What Sudan Can Learn from China's Development Model | Development | Not started | Not started | Not ready | No | No | Define what is adaptable and what should not be copied |
| B04 | Suakin, Arkawit, the Red Sea Coral, and Building Domestic Tourism | Culture / Development | Not started | Not started | Not ready | No | No | Recover notes and decide whether cultural preservation or tourism economics leads |
| B05 | Technology Parks for Sudan | Development | Not started | Not started | Not ready | No | No | Audit current Sudanese capacity and avoid real-estate-only “tech park” proposals |
| B06 | A Sudanese Sovereign Wealth Fund / Citizen Wealth Model | Development | Not started | Not started | Not ready | No | No | Recover the UBI/wealth-board note and establish governance, dividend, and investment options |
| B07 | What Degrees and Vocations Should Sudanese Study Abroad? | Development | Not started | Not started | Not ready | No | No | Recover the note and connect scholarships to capability gaps and return pathways |
| B08 | Statistics as Part of Development Planning | Development | Not started | Not started | Not ready | No | No | Recover note; connect CBS capacity, targets, dashboards, evaluation, and accountability |
| B09 | Urban Planning Across the Country | Development | In progress through A06 | Not started | Not ready | No | No | Decide whether to merge into A06 or retain as a broader national planning companion |
| B10 | Sudanese Historical Images | History | Not started | Not started | Not ready | No | No | Curate verified Sudan-only archival images with provenance and captions |
| B11 | Sudanese Historical Images of Resistance | History | Not started | Not started | Not ready | No | No | Recover note and define an evidence-based thematic series |
| B12 | Sudanese Film / Film and Television | Culture | Not started | Not started | Not ready | No | No | Recover idea-bank notes and define the cultural/industry thesis |
| B13 | Ijtihad and Sharia | Ideas | Not started | Not started | Not ready | No | No | Recover note and establish the precise question and evidentiary scope |
| B14 | Sport as National Identity Beyond Wrestling | Culture | Partly covered in A03 | Not started | Not ready | No | No | Decide whether A03 is sufficient or whether a national sports-policy article is warranted |

## Library analytical-summary queue

Every full Library summary should follow this method:

**What the work argues → evidence/data and methodology → recommendations → strongest contribution → weaknesses/assumptions → what happened afterward → what remains valid today → what Sudan should actually do → connections to other Kandaka research and Library works.**

### L01 — Uranium in Sudan

- **Exact source to catalog:** `Uranium in Sudan`, Ministry of Minerals / Geological Research Authority of Sudan presentation by Musb Osman Ebrahim and Hesham Aldeen Alkhar (38 pages). The catalog may also contain or describe it as `Uranium in Sudan: Way Forward`; reconcile the exact title/record before publishing.
- **Research:** Source reviewed
- **Draft:** Not started
- **Review:** Not ready
- **Approved:** No
- **Published:** No
- **Requirements:** State precisely that the document reports uranium occurrences in several regions and comparatively high uranium concentration in Uro phosphate, but does not establish proven commercial reserves, recoverable tonnes, development cost, or mine feasibility. Explain UNFC/resource classification needs. Analyze uranium extraction/export, possible phosphate byproduct recovery, nuclear-power relevance, environmental/community justice, and opportunity cost without overstating the evidence.
- **Next action:** Preserve/catalog the exact supplied file, reconcile its Library title and metadata, and draft the analytical summary from the actual document.

### L02 — Sudan's Infrastructure: A Continental Perspective

- **Authors:** Rupa Ranganathan and Cecilia Briceño-Garmendia
- **Research:** Source and key findings verified preliminarily
- **Draft:** Not started
- **Review:** Not ready
- **Approved:** No
- **Published:** No
- **Requirements:** Cover roads, rail, electricity, water, telecommunications, infrastructure's growth contribution, annual spending, inefficiency/waste, transport/water gaps, what happened afterward and during war, maintenance versus new megaprojects, and today's highest-return reconstruction choices. Connect to the Five-Year Plan, transport article, and megaproject scorecard.
- **Next action:** Retrieve the complete Library copy, confirm whether a full Kandaka treatment already exists, and produce the analytical summary.

### L03 — Sudanese Universities as Sites of Social Transformation

- **Author:** Linda S. Bishai
- **Research:** Source and thesis verified preliminarily
- **Draft:** Not started
- **Review:** Not ready
- **Approved:** No
- **Published:** No
- **Requirements:** Examine universities as sites of political/social change; expansion without adequate staff/resources; language and national-identity policies; effects on non-Arab communities; political control of students/faculty; women and marginalized communities; brain drain; and whether universities became engines of technical skill, research, industry, and national integration or primarily degree-producing institutions. Revisit the argument with later evidence.
- **Next action:** Retrieve the complete Library copy, verify no duplicate full treatment, and produce the analytical summary.

## Government and official-report acquisition stream

This is a permanent Library-acquisition stream, not a one-time search.

### Sudanese institutions to monitor

- Ministry of Finance and Economic Planning
- Ministry of Agriculture and Forestry
- Ministry of Industry and Trade
- Ministry of Energy and Petroleum
- Ministry of Minerals and Geological Research Authority of Sudan (GRAS)
- Ministry of Transport and Sudan Railways archives
- Ministry of Irrigation and Water Resources
- Ministry of Health
- Ministry of Education and Ministry of Higher Education
- Central Bureau of Statistics
- Central Bank of Sudan (CBOS) archives
- State governments and state planning/finance ministries
- Agricultural Research Corporation and other public research institutes
- Public corporations and industrial/agricultural authorities

### Known official leads to audit

| Source/report | Status | Why it matters | Next action |
|---|---|---|---|
| Sudan National Agriculture Investment Plan 2016–2020 | Candidate located | Productivity, exports, food security, poverty, agribusiness, women/youth, federal/state implementation | Retrieve official copy; verify metadata and Library duplication |
| 2025 Local Sustainable Development Report (Ministry of Finance) | Candidate located | Current/local planning and implementation | Retrieve and assess original data/analysis value |
| Ministry of Minerals 2026–2030 strategic vision | Candidate located | Geological survey, minerals database, refineries, proposed Minerals Bank | Retrieve full official document; distinguish strategy from publicity |
| Government 2026 economic priorities | Candidate located | Regional oilseed/vegetable-oil and livestock complexes; state youth employment | Locate primary plan/budget documents behind announcements |
| CBOS annual reports and trade/statistical digests | Ongoing priority | Imports, exports, crops, manufacturing, transport, borrowing, grants, development expenditure, monetary history | Build a complete dated archive with stable metadata |
| National Energy Assessment | Historical lead | Primary energy planning evidence | Locate exact edition and official/repository copy |
| Comprehensive Industrial Survey | Historical lead | Industrial baseline and performance | Locate exact edition and official/repository copy |
| Fertilizer-manufacturing studies | Historical lead | Feasibility and industrial-policy history | Identify titles, dates, authors, and later evaluations |
| Ministry of Finance annual reports | Historical lead | Budgets, development expenditure, debt, and plan execution | Build year-by-year acquisition list |

### Acquisition criteria

Prioritize documents containing original data, strategies, feasibility studies, surveys, evaluations, audits, historically important plans, and evidence of implementation or failure. Search for paired records: an early feasibility study plus a later evaluation may be more valuable than a current overview. Do not download everything a ministry publishes. Press releases generally remain research leads, not Library entries.

For every acquisition record: exact title; author/institution; date/version; original URL; local/R2 location; file format; language; page count; rights/access note; checksum if practical; Library duplication check; short relevance note; and status.

## Wider research-source streams

### Sudanese and regional sources

- Central Bureau of Statistics
- CBOS archives
- GRAS
- University of Khartoum, Sudan University of Science and Technology, University of Gezira, and other Sudanese university repositories
- Sudan Academy of Sciences
- State-government planning documents
- Agricultural Research Corporation
- Sudan Railways and other public-corporation archives
- Sudanese professional associations
- Sudanese journals, theses, conference proceedings, and diaspora scholarship
- Sudanese civil-society archives, Emergency Response Rooms, unions/cooperatives, and community organizations when relevant

### International and historical collections

- World Bank Documents & Reports
- IMF archives
- African Development Bank
- UN Economic Commission for Africa
- FAO, ILO, UNIDO, UN-Habitat, UNEP, UNESCO, and IAEA
- Arab Fund for Economic and Social Development
- Islamic Development Bank
- OPEC Fund
- African Union and regional economic/community institutions
- Older British/colonial archives and declassified administrative records
- Digitized newspaper, map, photographic, and institutional archives with clear provenance

## Website changes waiting for repository/GitHub write implementation

These changes are approved or requested but must be implemented and tested in the repository before they go live. This workplan itself does not publish them.

| Change | Research | Draft/implementation | Review | Approved | Published | Requirements / next action |
|---|---|---|---|---|---|---|
| Add History category in English and Arabic | Complete | Not started | Not ready | Yes | No | Add section indexes and bilingual navigation; preserve RTL |
| Add Culture category in English and Arabic | Complete | Not started | Not ready | Yes | No | Add section indexes and bilingual navigation; preserve RTL |
| Reclassify Funj article | Complete | Not started | Not ready | Yes | No | Move Ideas → History; title: `The Funj Sultanate: Three Centuries of Power on the Blue Nile`; preserve URLs or add redirects |
| Navigation update | Complete | Not started | Not ready | Yes | No | Order: Ideas · Development · History · Culture · News · Library in both languages |
| Featured homepage carousel/banner | Complete | Not started | Not ready | Yes | No | Keep Kandaka identity/hero; rotate Ideas, Development, History, Culture, and selected Images/Archive items; image, category, title, teaser, CTA; accessible controls and performance-conscious loading |
| Social sharing on articles | Complete | Not started | Not ready | Yes | No | Add appropriate sharing buttons and bilingual/RTL behavior; avoid intrusive tracking |
| Sudan-only image-feed filtering | Complete | Not started | Not ready | Yes | No | Exclude `South Sudan`, Juba, South Sudanese categories/titles and known false matches from Flickr/Wikipedia feeds; test live results rather than trusting query text alone |
| Article image/social metadata pipeline | Partial | Not started | Not ready | No | No | Ensure cover images drive article, Open Graph/social, and Pinterest previews with correct attribution |
| Kandaka Sudan News Hub — English and Arabic | Complete | Phase 1 implemented; testing complete | Awaiting repository/deploy review | Yes | No | Preserve Phase 1; review and deploy before beginning ranking/social phases |

### Kandaka Sudan News Hub — approved implementation record

**Objective:** Make Kandaka the broad, bilingual gateway for timely reporting about the Republic of Sudan—not South Sudan—without allowing the UN, any institution, any publisher, war reporting, or Khartoum to dominate. Kandaka must credit and link to original publishers; aggregation does not authorize full-text copying or automatic Kandaka editorial publication.

**Approved architecture:** Grow toward 60–100 reviewed Arabic and English sources across Sudanese/local journalism, diaspora and regional media, individual reporters, international journalism, official institutions, civil society/Emergency Response Rooms, universities/researchers, economic/data services, humanitarian sources, and carefully curated social/OSINT accounts. Arabic and English ingest native-language sources independently and should expose valuable reporting across the language boundary.

**Required processing:** Normalize URLs and metadata; contextually distinguish Sudan from South Sudan; classify topic and geography; label source/status; cluster repeated coverage of one event; prefer Sudanese original reporting as the cluster lead; seek corroboration for breaking/social claims; retain uncertainty; generate only short attributed summaries; and expose a prominent original-source link.

**Diversity rules for Phase 2:** Normally no more than two cards from one publisher among the first 15 and no more than three UN/international-institution primary cards among the first 20. Ranking should reward freshness, local reporting, corroboration, new information, and source/topic/geographic diversity. Required topics include politics, war/security, economy, business, agriculture, humanitarian, health, education, infrastructure, environment/weather, culture, sport, society, diaspora, diplomacy/international, and official. Required geography includes Khartoum, Darfur, Kordofan, Gezira, Red Sea, Kassala, Gedaref, Northern, River Nile, Blue Nile, Sennar, White Nile, National, and Diaspora.

**Media requirement:** News cards should support appropriate original photographs, video thumbnails, and social-media imagery, with source attribution, rights-aware handling, lazy loading, and fallbacks when media cannot safely be reused. Prefer permitted feed metadata, licenses, explicit permission, or platform embeds. Store source, attribution, reuse basis, alt/caption information, and retrieval context. Never use invented/AI documentary imagery for a current event, and do not imply that a thumbnail verifies a claim. Protect vulnerable people and avoid graphic or identifying imagery where unsafe.

**Phase 1 completed 2026-09-19:**

- Added a centralized bilingual source registry with source class and media policy, including disabled social/reporting candidates for a later permitted integration.
- Replaced the faulty keyword rule that treated `Juba` as positive Sudan evidence with contextual Sudan/South Sudan classification; legitimate cross-border Sudan stories remain eligible.
- Added normalized URLs, original publication dates, topic and geography tagging, source/status labels, duplicate clustering, Sudanese-source preference within clusters, related-source metadata, and corroboration counts.
- Added rights-aware RSS image metadata, attribution fields, responsive 16:9 rendering, lazy loading, error fallback, and a neutral Kandaka/Sudan news fallback.
- Excluded three existing South Sudan-only archive records by marking them draft with an auditable exclusion reason; nothing was deleted.
- Added automated tests for English/Arabic Sudan relevance, South Sudan exclusion and contextual retention, URL normalization, topic/geography, clustering preference, and media provenance. Eight tests pass.
- Updated the news workflow to run the tests before fetching and committing news.

**Remaining phases:** Phase 2 diversity ranking, chronological/ranked views, filters and cross-language discovery; Phase 3 curated social discovery, permitted embeds, verification/corroboration workflow; Phase 4 expansion toward 60–100 healthy inputs with monitoring of failures, false positives, duplicates, concentration and media errors.

**Status:** Phase 1 implemented and locally tested; review ready; approved for repository implementation, not yet deployed.  
**Next action:** Review the packaged Phase 1 repository changes, merge/push them to GitHub, allow Netlify to deploy, and verify English/Arabic desktop and mobile output before activating Phase 2.

## Image and visual requirements

1. Use authentic Sudan-related images wherever possible; avoid generic stock imagery that could represent any country.
2. Verify source, creator, license, attribution text, and whether edits/crops are allowed before publication.
3. Exclude South Sudan material unless the article explicitly concerns South Sudan. Do not rely on keyword matching alone; visually and contextually verify the subject.
4. Every article should have a horizontal cover suitable for the site and social preview. Create a separate vertical Pinterest treatment when useful rather than placing headline text over the article cover.
5. Captions must say what is actually shown and avoid using illustrative images as proof of historical continuity, mineral reserves, institutional performance, or current conditions.
6. Prefer images showing Sudanese agency, production, culture, institutions, landscapes, and historical evidence over repetitive war/poverty imagery unless conflict is the subject.
7. Store stable image metadata alongside the article: source URL, creator, license, attribution, alt text, caption, crop notes, and download date.
8. Specific pending needs are recorded under A01, A03, A04, and A05. The transport article will need a clear comparison graphic or map only if it materially improves the argument.

## Research Watch and update policy

### Research Watch

- Maintain a permanent watch on Sudanese ministry, CBOS, CBS, GRAS, university, public-corporation, international-institution, and historical-archive sources.
- Favor new primary documents, newly digitized historical material, revised datasets, feasibility/evaluation pairs, and material that corrects or tests an existing Kandaka claim.
- Record a lead even when it is not acquired immediately, but do not call it a Library holding until the actual file is secured and cataloged.
- For current affairs, humanitarian organizations, prices, laws, officeholders, programs, and operating access, refresh research immediately before publication.
- When a new source changes a thesis, update the article openly; do not force the evidence to fit the earlier idea.

### Progress updates

- Updates should state: what changed, what was verified, important findings/numbers, blockers, decisions needed, and the next concrete action.
- Never imply background work is continuing when no automation or active task is actually running.
- The earlier concept was a concise daily research update and an editorial advancement every three days. Treat these as policy proposals; verify any current automation separately before relying on it.
- When the user says `go ahead`, execute the available work and return a substantive result rather than only describing the next step.

### Publication safety

- No automatic publication from the research watch.
- No draft is approved merely because its thesis is established or the research is complete.
- No translation or scheduling substitutes for editorial approval.
- Verify the live page after publication and confirm bilingual links, images, categories, metadata, and share previews.

## Status dashboard

| Workstream | Research | Draft | Review | Approved | Published | Immediate next action |
|---|---|---|---|---|---|---|
| A01 Five-Year Plan | Complete/refresh | Complete | Awaiting review | No | No | Produce updated clean review copy |
| A02 Sudan Planned Before | In progress | Complete first draft | Not ready | No | No | Final quantified timeline and fact-check |
| A03 Wrestling | Complete/refresh | Complete reported | Awaiting review | No | No | Consolidate and verify review copy |
| A04 Livestock/fisheries | Complete/refresh | Complete reported | Awaiting review | No | No | Consolidate and update evidence |
| A05 Monetary reform | Complete/refresh | Partial/substantial | Not ready | No | No | Create one consolidated draft |
| A06 Public transport/urban planning | Complete enough | Not started | Not ready | No | No | Draft article |
| A07 Independence flag | Preliminary | Not started | Not ready | No | No | Research and outline |
| A08 Country-name question | Preliminary | Not started | Not ready | No | No | Research neutral criteria |
| A09 Trustworthy aid/delivery | Refresh required | Not started | Not ready | No | No | Current organization audit |
| A10 Women/children protection service | Not started | Not started | Not ready | No | No | Legal/institutional research |
| A11 Transformative megaprojects | Preliminary | Not started | Not ready | No | No | Build comparative scorecard |
| L01 Uranium in Sudan | Source reviewed | Not started | Not ready | No | No | Reconcile file/catalog metadata and summarize |
| L02 Infrastructure | Preliminary | Not started | Not ready | No | No | Retrieve full source and check duplication |
| L03 Universities | Preliminary | Not started | Not ready | No | No | Retrieve full source and check duplication |
| History + Culture categories | Complete | Not implemented | Not ready | Yes | No | Implement and test in repository |
| Other website batch | Complete/partial | Not implemented | Not ready | Mixed above | No | Implement only approved changes and test |
| Kandaka Sudan News Hub | Complete | Phase 1 implemented and tested | Awaiting deploy review | Yes | No | Push/merge Phase 1, verify live feeds/media/RTL, then begin Phase 2 |

## Change log

- **2026-09-19:** Implemented Phase 1 of the approved Kandaka Sudan News Hub in the supplied repository: bilingual source registry; contextual Sudan/South Sudan classification; topic/geography/source/status metadata; duplicate clustering; local-source preference; attributed lazy-loaded imagery and safe fallback; three historical South Sudan-only exclusions; workflow validation; and eight passing automated tests. No editorial article was approved or published, and no deployment was performed.
- **2026-08-29:** Created the persistent workplan from the `Kandaka Research Watch` conversation and the existing `KANDAKA_EDITORIAL.md` framework. Recorded all known active drafts, named backlog items, Library summaries, official/acquisition sources, pending site work, images, cadence, research-watch policy, and approval safeguards. No content was published.
